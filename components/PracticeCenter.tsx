...
export const PracticeCenter: React.FC<PracticeCenterProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="p-6 max-w-2xl w-full mx-auto bg-white rounded-xl shadow-2xl space-y-4"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-center">
          <h2 className="text-2xl font-bold text-gray-800">{t('dashboard.practiceCenter')}</h2>
...
const TabButton: React.FC<{icon: React.ReactNode, label: string, isActive: boolean, onClick: () => void}> = 
  ...
  <button
    onClick={onClick}
    className={`flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium border-b-2
      ${isActive 
